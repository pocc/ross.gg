---
title: "DNS Caches"
pubDatetime: 2021-10-11T19:24:42Z
description: "How to view and flush DNS caches in browsers, Windows, Linux, and macOS."
tags:
  - dns
  - caching
  - networking
...
_This article shows how to get browser and OS DNS caches._
...
On Windows, you can use the `ipconfig /displaydns` command:
...
```bash
time=$(date "+%F %T")
systemd-resolve cloudflare.com >/dev/null
systemctl kill -s USR1 systemd-resolved
journalctl -b -0 --since "$time" -u systemd-resolved | grep " IN "
```
...
On 10.14, you can use mDNSResponder to get data. You need to make sure that records are not private so that you can see them.
...
More information can be found with this [Stack Overflow question](https://stackoverflow.com/questions/38867905/how-to-view-dns-cache-in-osx).
